...
class AlertCheckLimitServiceTest {
    ...
    void setUp() {
        ...
        existingLimit.setKey(UUID.randomUUID());
    ...
    void testGetLimit_NotFound() {
        ...
        AlertCheckLimit result = alertCheckLimitService.getLimit("aquark_data", "rain_d");
    ...
    void testGetLimit_Found() {
        ...
        assertEquals(existingLimit.getKey(), result.getKey());
    }
    ...
    void testInsertLimit_NewLimit() {
        ...
        when(alertCheckLimitDataAccess.save(any(AlertCheckLimit.class))).thenAnswer(invocation -> {
            ...
            saved.setKey(UUID.randomUUID());
            return saved;
        });

        AlertCheckLimit result = alertCheckLimitService.insertLimit("aquark_data", "rain_d", 12.3);
    ...
    void testInsertLimit_ExistingLimit() {
        ...
        AlertCheckLimit result = alertCheckLimitService.insertLimit("aquark_data", "rain_d", 99.9);
        ...
        assertEquals(existingLimit.getKey(), saved.getKey());
    ...
    @Test
    void testUpdate() {
        when(alertCheckLimitDataAccess.save(existingLimit)).thenReturn(existingLimit);

        AlertCheckLimit result = alertCheckLimitService.update(existingLimit);

        assertEquals(existingLimit, result);
        verify(alertCheckLimitDataAccess, times(1)).save(existingLimit);
    }
    ...
    void testGetLimit_All() {
        ...
        List<AlertCheckLimit> result = alertCheckLimitService.getLimit();

        assertEquals(1, result.size());
        assertEquals(existingLimit.getKey(), result.getFirst().getKey());
    }

    @Test
    void testDeleteLimit() {
        alertCheckLimitService.deleteLimit(existingLimit);

        verify(alertCheckLimitDataAccess, times(1)).delete(existingLimit);
    }
}
